...
y0=float(-0.025)
for row in data: 
    if float(row[10])==1:#if float(row[1]) == float(y0) and .009<float(row[2])<.01: #and float(row[0])==float(wavenumber): #phonon at depth 0
...
py.image.save_as({'data': data}, 'C:\Users\Suryabrata\Dropbox\Yale\Summer 2015\McKinsey Research\SimulationData\Wavenumber_Z0\Final Data\plots\heatmaps\heatmap_combined_noprompt.png')
