...
cyl_height=float(1)
sim_time=float(5)
vacuum_gap=float(0.1)


#___________________________________________________________________

#array initialization
t=linspace(0,sim_time,sim_time*100000) #1000000 points per second
transmission=[]
bolometer=[]
del_t=t[2]-t[1]
func1=[] #Function 1: absorbance/transmission ratio vs wavenumber

#__________________________________________________________________'
print "Currently Processing Loop: 1"
for n in range(99):
    for zz in xrange(1,10): #iterating over radii
        ...
        for j in xrange(2,len(t)): #iterating time values
            for i in xrange(n_particle): #iterating particle values
                ...
                if float(a[i,5,j-1])>cyl_height/2+vacuum_gap and float(a[i,5,j-2])<=cyl_height/2+vacuum_gap: #bolometer
                    bolometer.append([wavenumber,cyl_rad,t[j],a[i,1,j-1],a[i,3,j-1],a[i,5,j-1],a[i,2,j-1],a[i,4,j-1],a[i,6,j-1],a[i,7,j-1],a[i,8,j-1]])
                    vel2=array([0,0,0])
                ...
                if float(a[i,1,j-1])**2.0 + float(a[i,3,j-1])**2.0 > cyl_rad**2.0: #side boundary
                    ...
                    if newstate != 3:
                        ...
                        if newstate==3:
                            vel2=array([0,0,0])
                            transmission.append([wavenumber,cyl_rad,t[j],a[i,1,j-1],a[i,3,j-1],a[i,5,j-1],a[i,2,j-1],a[i,4,j-1],a[i,6,j-1],a[i,7,j-1],a[i,8,j-1]])
                    if newstate != 3:
                        vel2=bounce(vel1,normal,a[i,7,j-1],newstate)
                        a[i,8,j]=1
        ...
        func1.append([wavenumber, z0, len(bolometer),len(transmission)]) 
    ...
    bolometer_save="/global/homes/e/epease/bolometer/"+str(n)+".csv"
    transmission_save="/global/homes/e/epease/transmission/"+str(n)+".csv"
    func1_save="/global/homes/e/epease/func/"+str(n)+".csv"           
    savetxt(bolometer_save, bolometer, delimiter=",")
    savetxt(transmission_save, transmission, delimiter=",")
    savetxt(func1_save, func1, delimiter=",")
    print "Currently Processing Loop: " + str(n+2)
