        ...
        for j in xrange(2,len(t)): #iterating time values
            for i in xrange(n_particle): #iterating particle values
                ...
                if float(a[i,5,j-1])>cyl_height/2.0+vacuum_gap and float(a[i,5,j-2])<cyl_height/2.0+vacuum_gap: #bolometer
                    if newstate==3:
                        bolometer.append([wavenumber,z0,t[j],a[i,1,j-1],a[i,3,j-1],a[i,5,j-1],a[i,2,j-1],a[i,4,j-1],a[i,6,j-1],a[i,7,j-1],a[i,8,j-1]])
                    vel2=array([0,0,0])
                if float(a[i,5,j-1])>cyl_height/2.0 and float(a[i,5,j-2])<cyl_height/2.0: #top boundary
                    ...
                    if newstate!=3:
                        ...
                        if newstate==3:
                            ...
                            top.append([wavenumber,z0,t[j],a[i,1,j-1],a[i,3,j-1],a[i,5,j-1],a[i,2,j-1],a[i,4,j-1],a[i,6,j-1],a[i,7,j-1],a[i,8,j-1]])
                        else:
                ...
                if float(a[i,1,j-1])**2.0 + float(a[i,3,j-1])**2.0 > cyl_rad**2.0 and float(a[i,1,j-2])**2.0 + float(a[i,3,j-2])**2.0 < cyl_rad**2.0: #side boundary
                    ...
                    if newstate != 3:
                        ...
                        if newstate==3:
                            vel2=array([0,0,0])
                            transmission.append([wavenumber,z0,t[j],a[i,1,j-1],a[i,3,j-1],a[i,5,j-1],a[i,2,j-1],a[i,4,j-1],a[i,6,j-1],a[i,7,j-1],a[i,8,j-1]])
                    else:
                        vel2=array([0,0,0])
                        atom_side.append([wavenumber,z0,t[j],a[i,1,j-1],a[i,3,j-1],a[i,5,j-1],a[i,2,j-1],a[i,4,j-1],a[i,6,j-1],a[i,7,j-1],a[i,8,j-1]])
                    if newstate != 3:
                        vel2=bounce(vel1,normal,a[i,7,j-1],newstate)
                        a[i,8,j]=1
...
bolometer_save="/global/homes/e/epease/bolometer/bolometer_"+str(randloopnumber)+".csv"
transmission_save="/global/homes/e/epease/transmission/transmission_"+str(randloopnumber)+".csv"
top_save="/global/homes/e/epease/top/top_"+str(randloopnumber)+".csv"    
atom_side_save="/global/homes/e/epease/atomside/atomside_"+str(randloopnumber)+".csv"       
